...
pg_event_filter(void *_, SDL_Event *event)
{
    ...
    if (event->type == SDL_WINDOWEVENT) {
        ...
        if (!_pg_last_keydown_event) {
            /* Use malloc and not PyMem_New because the event filter may be
             * called from a thread without GIL */
            _pg_last_keydown_event = malloc(sizeof(SDL_Event));
        }
